...
void missileMove(){
	if(isMissile == 1){
		GLCD_displayChar(missile_posX, missile_posY, ' ');
		missile_posX += 1;
		GLCD_displayChar(missile_posX, missile_posY, missile_shape);
		if(missile_posX >=320){
			isMissile = 0;
		}
	}				
...
void meteoCrash(){
	if(meteo_posY == missile_posY && meteo_posX <=missile_posX){
		isMissile = 0;
		GLCD_displayChar(meteo_posX, meteo_posY, ' ');
		GLCD_displayChar(missile_posX, missile_posY, ' ');			
		createMeteo();
