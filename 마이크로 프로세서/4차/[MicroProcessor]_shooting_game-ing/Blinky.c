...
void gameOver(){
	...
	if(isPassed == 0){
		...
		GLCD_displayStringLn(Line2, "  You Missed Meteo ");
	}
	else{
		GLCD_displayStringLn(Line1, "     GAME OVER  ");
...
void fighterCrash(){
	if((meteo_posY+10 >= fighter_posY && meteo_posY -10<= fighter_posY) &&(meteo_posX <= fighter_posX +20)){
		gameOver();
	}
}
